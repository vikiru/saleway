...
public class WebhookController {
  ...
  @Autowired private OrderClientService orderClientService;
  ...
  public ResponseEntity<String> handleStripeWebhook(
    ...
    if ("charge.refunded".equals(event.getType())) {
      ...
      try {
        ...
        // In this basic implementation we assume checkout_session_id is in metadata or we retrieve
        // it
        // Or for now, we just pass the Payment Intent ID, but our DB queries by stripe_session_id.
        // Usually, when creating the session, Stripe links it. Let's assume metadata holds the
        // session ID.
