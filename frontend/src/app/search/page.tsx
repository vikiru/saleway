          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-x-8 gap-y-10">
            <form className="hidden lg:block">
              ...
              {Object.entries(filters).map(([sectionName, sectionItems]) => (
                <div key={sectionName} className="border-b border-gray-200 py-6">
                  <h3 className="-my-3 flow-root">
                    <div className="py-3 bg-white w-full flex items-center justify-between text-sm text-gray-400 hover:text-gray-500">
                      <span className="font-medium text-gray-900 capitalize">{sectionName}</span>
                    </div>
                  </h3>
                  <div className="pt-6">
                    <div className="space-y-4">
                      {sectionItems.map((option) => (
                        <div key={option.value} className="flex items-center">
                          ...
                          />
                          <label htmlFor={`filter-${sectionName}-${option.value}`} className="ml-3 text-sm text-gray-600">
                            {option.label}
                          </label>
                        </div>
                      ))}
