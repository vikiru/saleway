...
export function FilterSection({
  ...
  const toggleFilter = (list: string[], setList: (value: string[]) => void, item: string) => {
          ...
          {categories.map((category) => (
            <div className="flex items-center space-x-2" key={category.id}>
              <Checkbox
                ...
                id={`cat-${category.id}`}
                onCheckedChange={() => toggleFilter(selectedCategories, onCategoriesChange, category.id)}
              />
              <label
          ...
          {brands.map((brand) => (
            <div className="flex items-center space-x-2" key={brand.id}>
              <Checkbox
                ...
                id={`brand-${brand.id}`}
                onCheckedChange={() => toggleFilter(selectedBrands, onBrandsChange, brand.id)}
              />
              <label
                className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
